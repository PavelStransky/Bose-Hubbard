...
class BoseHubbard:
    ...
    def build_hamiltonian_block(self, l):
        ...
        for i1 in range(dim):
            state1 = self.basis_states[i1]

            for i2 in range(dim):
                state2 = self.basis_states[i2]
...
if __name__ == "__main__":
    ...
    N_max = 100        # max bosons per site
    N_total = 100      # fixed total boson number
    translational_sector = 0       # momentum sector (integer from 0 to N_sites-1)
    parity_sector = 1
    ...
    np.savetxt(f"Y output k={translational_sector}, parity={parity_sector}.csv", eigenvals)
