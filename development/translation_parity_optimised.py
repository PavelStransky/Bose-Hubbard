...
def momentum_eigenstates(N_sites, N_max, orbits, k, parity_sector=0):
    basis_k = []
    representative_states = []

    print(f"Constructing momentum eigenstates for k = {k}, parity = {parity_sector} ...")
    start_time = time.time()
    ...
    for orbit in alive_it(orbits):
        ...
        if len(orbit) > 1:
            for r, state in enumerate(orbit[1]):
                phase = np.exp(-2j * np.pi * k * r / N_sites)
                ket = tensor([basis(N_max+1, n) for n in state])
                psi_parity = phase * ket if psi_parity is None else psi_parity + phase * ket    

        state = orbit[0][0]  # representative Fock state of the orbit

        if psi_parity is None:
            if parity_sector != -1:
                basis_k.append(psi.unit()), representative_states.append(state)
            continue

        even_psi = (psi + psi_parity).unit()
        odd_psi = (psi - psi_parity).unit()

        if parity_sector >= 0:  # even parity
            basis_k.append(even_psi), representative_states.append(state)
        elif parity_sector <= 0:  # odd parity
            basis_k.append(odd_psi), representative_states.append(state)

    print(f"Constructed {len(basis_k)} basis states in {time.time() - start_time:.2f} seconds.")

    return basis_k, representative_states
...
def build_hamiltonian(N_sites, N_max, N_total, basis_k, representative_states, J, U, mu):
    """ Build the Bose-Hubbard Hamiltonian in the reduced momentum basis. """

    print("Building Hamiltonian in reduced momentum basis...")
    start_time = time.time()
    ...
    # Initialize sparse Hamiltonian in reduced basis
...
if __name__ == "__main__":
    ...
    for k in range(N_sites):
        ...
        for parity in parities:        
            ...
            basis_k, representative_states = momentum_eigenstates(N_sites, N_max, orbits, k, parity)
            H = build_hamiltonian(N_sites, N_max, N_total, basis_k, representative_states, J, U, mu)
            ...
            print("-" * 50)
            print(f"Total time for k = {k}, parity = {parity}: {time.time() - start_time_full:.2f} seconds.")
